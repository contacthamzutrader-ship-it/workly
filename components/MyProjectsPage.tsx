...
export type ProjectsTab = "all" | "active" | "pending" | "completed" | "cancelled";
...
export default function MyProjectsPage({ initialTab = "all" }: { initialTab?: ProjectsTab }) {
  const { user } = useAuth();
  const [tab, setTab] = useState<ProjectsTab>(initialTab);
  const [counts, setCounts] = useState<Record<ProjectsTab, number> | null>(null);
  ...
  const emptyHints: Record<ProjectsTab, string> = {
