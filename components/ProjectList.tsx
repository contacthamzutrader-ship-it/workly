...
export default function ProjectList({ load, emptyHint }: { load: () => Promise<Task[]>; emptyHint: string }) {
  ...
  if (tasks === null) {
    return <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{[1,2,3].map((i) => <div key={i} className="h-64 animate-pulse rounded-3xl border border-ink-100 bg-white" />)}</div>;
  }
  ...
  if (tasks.length === 0) {
    return (
      <div className="surface py-16 text-center">
        ...
        <h3 className="mt-5 text-xl font-black text-ink">Nothing here yet</h3>
  ...
  return (
    <div className="space-y-3">
      {tasks.map((t) => (
        <Link key={t.id} href={`/tasks/${t.id}`} className="group flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-ink-100 bg-white p-4 shadow-card transition hover:-translate-y-0.5 hover:border-brand/30 hover:shadow-card-hover">
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-black text-ink group-hover:text-brand-dark">{t.title}</p>
          ...
          <div className="flex shrink-0 items-center gap-4">
            <div className="text-right">
              <p className="text-base font-black text-ink">{formatPKR(t.heldAmount ?? t.budget)}</p>
              <span className={`mt-0.5 inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[11px] font-bold ${STATUS_CHIP[t.status] || "bg-ink-50 text-ink-500 border-ink-100"}`}>{t.status.replace("_", " ")}</span>
            </div>
