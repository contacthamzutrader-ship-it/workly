...
import { Award, Check, Pencil, Plus, Save, Sparkles, Trash2, X } from "lucide-react";
...
export default function SkillsPage() {
  ...
  useEffect(() => { if (!loading && !user) router.replace("/login?redirect=/skills"); }, [loading, user, router]);
      ...
      <div className="page-shell max-w-3xl">
        <div className="overflow-hidden rounded-[32px] bg-[#00501F] p-6 text-white shadow-elevated sm:p-8">
          <div className="flex items-center gap-4">
            <div className="grid h-14 w-14 place-items-center rounded-2xl bg-brand"><Award className="h-7 w-7" /></div>
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-brand-300">Account centre</p>
              <h1 className="mt-1 text-2xl font-black tracking-[-0.03em]">Skills</h1>
              <p className="mt-1 text-sm text-white/55">Add, edit or remove the services and tools you offer.</p>
            </div>
          </div>
        </div>
