    ...
    <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center bg-gray-50 px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-ink-500">Join Workly and start posting or doing tasks</p>
        </div>
        ...
        <div className="mt-8 rounded-2xl border border-ink-100 bg-white p-8 shadow-card">
          <form onSubmit={submit} className="space-y-4">
            ...
            <Button type="submit" disabled={busy} className="w-full rounded-xl py-3">
              {busy ? <span className="flex items-center gap-2"><span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" /> Creating account...</span> : "Create account"}
            </Button>
          </form>
        ...
        <p className="mt-6 text-center text-sm text-ink-500">
          ...
          <Link href="/login" className="font-semibold text-brand transition hover:text-brand-dark">Sign in</Link>
        </p>
      </div>
    </div>
  );
